...
public class ProviderDAO {
	...
	// Find by id
	public List<ProviderModel> findListProvider(int idUser) {
		List<ProviderModel> listProvider = new ArrayList<ProviderModel>();
		connect = instance.getDataBaseConnectionPool();
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			pstm = connect.prepareStatement("SELECT * FROM provider WHERE idUser =?");
	...
	public void addProvider(ProviderModel providerModel, int idUser) {
		PreparedStatement pstm = null;
		try {
			connect = instance.getDataBaseConnectionPool();
			pstm = connect.prepareStatement("insert into provider(nameProvider,address,phone, idUser) values (?, ?, ?, ? )");
	...
	public void deleteProvider(int idProvider) {
		...
		try {
			connect = instance.getDataBaseConnectionPool();
			pstm = connect.prepareStatement("DELETE FROM provider WHERE idProvider=?");
	...
	public ProviderModel getProviderById(int idProvider) {
		...
		try {
			preparedStatement = connect.prepareStatement("select * from provider where idProvider=?");
